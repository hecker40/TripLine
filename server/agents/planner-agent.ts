...
import { planDetailedTrip, type PlanningProgress } from "./detailed-planner";
...
export class PlannerAgent {
  ...
  generateDetailedItinerary(
    ...
    return planDetailedTrip(this.provider, preferences, progress);
  }
      ...
      const days = itinerary.days.map((day) => ({
        ...
        estimatedDailyCost:
          day.activities.reduce(
            (sum, activity) => sum + Math.round(activity.estimatedCost * 100),
            0,
          ) / 100,
      }));
      const totalEstimatedCost =
        days.reduce(
          (sum, day) => sum + Math.round(day.estimatedDailyCost * 100),
          0,
        ) / 100;
      return validateItinerary(
        { ...itinerary, days, totalEstimatedCost },
        preferences,
      );
    } catch {
      throw new AppError(
        "INVALID_AI_OUTPUT",
        "The planner returned an inconsistent itinerary. Please try again.",
        502,
      );
    }
  }
