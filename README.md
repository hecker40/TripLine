...
Each date is independently checked for its pace-specific minimum blocks (4 relaxed / 6 balanced / 8 packed), at least two visits/explorations and two meal blocks, overnight lodging on non-final dates, chronology and transfer buffers. Durations and cost totals are calculated from the structured times/prices. A deterministic scheduler enforces the selected wake-up time and adds mode/distance-aware estimated transfer gaps between visits (10 minutes minimum when locations differ but coordinates are unknown). Existing transit blocks count toward the gap, rather than bypassing or doubling it. It shifts proposed blocks forward, preserves durations/costs, labels changes, and rejects days that cannot fit before midnight. The departure date cannot include an extra overnight hotel charge. The evaluator checks these buffers too, and the Adapter rejects insufficient-transfer repairs without replacing the prior itinerary. These are planning estimates, not verified routes or train schedules. A failed day gets one focused retry with its draft and validation feedback; successfully built days are not regenerated for that retry. Incomplete trips are not published. The whole-trip evaluator and Critic still run afterward. This validates structure and useful coverage, not venue opening hours or live train feasibility.
...
```dotenv
# Default endpoint is already integrated; override only for a different account.
BOOKING_MCP_URL=https://demandapi-mcp.booking.com/v1/mcp/8132308
BOOKING_BEARER_TOKEN=
```

The endpoint is built into the server for affiliate **8132308**, so this integration survives Git/Vercel deployment without copying local files. Only the bearer token is still required from the matching Booking.com partner account; an OpenAI key cannot authenticate Booking.com. Legacy `BOOKING_AFFILIATE_ID` overrides remain supported. If both URL and ID are configured they must match. Only the official HTTPS Booking.com MCP host/path is accepted, and redirects are refused before credentials can reach another host. See [official setup](https://developers.booking.com/mcp-server/docs/get-started) and [implementation guide](https://developers.booking.com/mcp-server/docs/implementation-guide). Without these credentials, the UI explicitly disables live MCP search but offers a separate, correctly labelled manual Booking.com search link with the generated dates/party. That link is **not** live MCP data.
